...
<?php 
	...
	if(isset($_POST['submit']))
	{
		...
		if($_FILES['excelFile']['name']!="")
		{
			...
			$count =0;
			echo "<table class=\"table table-bordered\" align=center><tr><th>Uname</th><th>Password</th><th>First Name</th><th>Last Name</th><th>DOB<br>(YYYY-MM-DD)</th><th>DOJ<br>(YYYY-MM-DD)</th><th>Exp<br>(X yrs- Y months)</th><th>Blood Group</th><th>e-Mail</th><th>Mobile No</th><th>Emergency Contact</th><th>Present Addr</th><th>Permanent Addr</th><th>Class</th><th>Remarks</th></tr>";			
			for($i=1;$i<=$data->sheets[0]['numRows'];$i++)
			{
				...
				$dob =$data->sheets[0]['cells'][$i][5];
				$doj =$data->sheets[0]['cells'][$i][6];
				$exp =$data->sheets[0]['cells'][$i][7];
				$blood =$data->sheets[0]['cells'][$i][8];
				$email =$data->sheets[0]['cells'][$i][9];
				$mobile =$data->sheets[0]['cells'][$i][10];
				$emergency_mob =$data->sheets[0]['cells'][$i][11];
				$present_addr =$data->sheets[0]['cells'][$i][12];
				$permanent_addr =$data->sheets[0]['cells'][$i][13];
				$class =$data->sheets[0]['cells'][$i][14];
				$remarks =$data->sheets[0]['cells'][$i][15];
				$query="INSERT INTO `oams`.`fac_users`(uname,pass1,first,last,dob,doj,exp,blood,email,mobile,emergency_mob,present_addr,permanent_addr,class,remarks)VALUES('".$uname."','".$pass1."','".$first."','".$last."','".$dob."','".$doj."','".$exp."','".$blood."','".$email."','".$mobile."','".$emergency_mob."','".$present_addr."','".$permanent_addr."','".$class."','".$remarks."')";
				$res = mysql_query($query);
				$count++;
				echo "<tr><td>$uname</td><td>$pass1</td><td>$first</td><td>$last</td><td>$dob<br></td><td>$doj</td><td>$exp</td><td>$blood</td><td>$email</td><td>$mobile</td><td>$emergency_mob</td><td>$present_addr</td><td>$permanent_addr</td><td>$class</td><td>$remarks</td></tr>";				
			}
			echo "</table>";			
			if($count)
			{
...
<table class="table table-bordered" align=center><tr><th>Uname</th><th>Password</th><th>First Name</th><th>Last Name</th><th>DOB<br>(YYYY-MM-DD)</th><th>DOJ<br>(YYYY-MM-DD)</th><th>Exp<br>(X yrs- Y months)</th><th>Blood Group</th><th>e-Mail</th><th>Mobile No</th><th>Emergency Contact</th><th>Present Addr</th><th>Permanent Addr</th><th>Class</th><th>Remarks</th></tr></table>
